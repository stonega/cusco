...
The live reasoning preview reserves exactly three single-line row widgets as soon as it appears. Blank slots retain their normal line height, and incoming lines fade through widget opacity inside those slots, so neither initial fill nor later rollover changes the preview's layout footprint. Once all three slots are occupied, later reasoning lines rotate through the same widgets in place. The outer reasoning body also disables its size-changing reveal transition while the live preview is active, then restores the normal expandable motion after completion. Its “Reasoning” header remains hidden during the live preview and returns afterward, and each paced update still emits the normal stream-frame callback.
