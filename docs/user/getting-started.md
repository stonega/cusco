...
Workspace preferences include a prompt library, reusable agent profiles, conversation folders, plugin tool descriptors, and optional MCP server configs. The Skills preferences page discovers installed skills from `~/.agents/skills`, where each skill folder contains `SKILL.md`. Enable skills there, then select them from the composer skill menu for a chat. Selected skills are sent as instruction context for the response and Cusco records a visible transcript note; skill files are not executed.
