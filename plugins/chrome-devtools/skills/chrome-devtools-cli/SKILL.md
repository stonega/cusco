...
description: Use the Chrome DevTools CLI for requested shell scripts or terminal browser automation. Ordinary browser inspection in Cusco uses the connected MCP tools.
---
...
chrome-devtools get_heapsnapshot_class_nodes "./snap.heapsnapshot" 0 # Inspect class instances using the numeric class ID from details
chrome-devtools get_heapsnapshot_details "./snap.heapsnapshot" # Snapshot aggregates and class IDs
...
```bash
chrome-devtools install_pwa "https://example.com/" "https://example.com/" # Install PWA (install URL, manifest ID)
...
Commands depend on the installed release and enabled categories. Check
`chrome-devtools --help` and `chrome-devtools start --help` before using optional
commands; not every MCP flag or tool is supported by the CLI. Enable supported
experimental tools with their corresponding flag during `start`.
