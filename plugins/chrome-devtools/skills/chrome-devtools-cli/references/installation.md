...
configuration; it does not inherit the MCP preset or its browser session.
...
```sh
npx -y --package=chrome-devtools-mcp@1.9.0 chrome-devtools start --isolated --workspace=/absolute/path/to/project --no-usage-statistics --no-performance-crux
```
