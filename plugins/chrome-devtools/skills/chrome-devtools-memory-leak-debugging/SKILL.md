...
- **Common Culprits:** Look for detached DOM nodes, unhandled closures, global variables, event listeners not being removed, and caches growing unbounded. Detached DOM nodes may be intentional caches; inspect ownership and lifecycle before changing them. Preserve intentional caches and clarify only when their purpose remains uncertain.
