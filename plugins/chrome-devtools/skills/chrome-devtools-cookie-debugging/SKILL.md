---
name: chrome-devtools-cookie-debugging
...
Page-scoped calls below require `pageId`; obtain it from `list_pages` or `new_page`.
Inspect cookie presence and attributes without copying session token values into chat.
