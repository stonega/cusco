...
## Cusco Connector Scope

When this skill runs in Cusco through GNOME Online Accounts, Gmail access is
read-only. Available actions are mailbox search, message reads, and thread reads.
You may draft reply text in the conversation, but do not claim to send, archive,
trash, move, label, or otherwise change email because the native connector does
not expose those operations.

Use only the native Gmail tools (`read_latest_email`, `search_emails`,
`search_email_ids`, `batch_read_email`, and `read_email_thread`) to access the
mailbox. Never fall back to Bash, local file tools, D-Bus commands, browser or
computer control, web search, direct Gmail HTTP requests, or manual IMAP code.
Never retrieve, print, inspect, copy, or place a GNOME Online Accounts access
token in a command or model-visible output. If the native Gmail tools are not
listed, stop and tell the user to connect Gmail from **Plugins → Gmail** or
restart Cusco after updating it.

For a request to read the latest email, call `read_latest_email` once with an
empty object. Add a narrow `query` only when the user specified a mailbox scope.
Do not enumerate unrelated tools, inspect configuration, or fetch additional
messages unless the user asked for them.
...
If thread or inbox data is missing, say that Gmail access may be unavailable or scoped to the wrong account and ask the user to reconnect or clarify which mailbox or thread should be used. This is an explanation-only fallback; do not try another access path.
