...
export function buildAgentModeSystemPrompt(tools, {
    ...
    const computerUseInstruction = hasComputerStep
        ? [
            ...
            'When computer_step reports autoZoom.applied, the attached image is the single automatically enlarged view of a localized post-click change. Use the returned region observationId and local 0..1000 coordinates directly; click the intended target center, do not request a full screenshot, and do not manually add the root offset. When retainedRegion.applied and likelyCoordinateMiss are true, keep the current control state and retry once at a visibly different center point inside that same retained crop. If visualStateCycleDetected is true, stop repeating the trigger and select from the current enlarged state or change to a semantic or keyboard strategy.',
